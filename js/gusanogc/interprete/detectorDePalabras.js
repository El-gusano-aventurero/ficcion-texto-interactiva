var detectorDePalabras=
{
	...
	buscar_conexiones_en_escenario:function(pl)
	{
		...
		for (key in guion.escenarioActual.conexiones)
		{
			...
			salQueBusco=pl.toLowerCase();
			salQueBusco=funcionesDeCadena.quitar_acentos(salQueBusco);
			
			if ((sal.startsWith(salQueBusco))&&(salQueBusco.length>=1))
			{
				return sal;
			}
		}
	...
	comprobar_si_es_pre_de:function(ultimoEncontrado,pl)
	{
		...
		{	
				var busqueda=detectorDePalabras.buscar_objeto_por_nombre(ultimoEncontrado.valor,contenido.objetos,[]);
				
				for (obje in busqueda)
				{
					preDeObjeto=busqueda[obje].preDe.toLowerCase();
					preDeObjeto=funcionesDeCadena.quitar_acentos(preDeObjeto);
