function Escenario(dat,key)
{
	...
	this.mostrarEntradaUsuario=true
	if("mostrarEntradaUsuario" in dat)
	{
		this.mostrarEntradaUsuario=dat.mostrarEntradaUsuario;
	}
	...
	if ("descripcionCorta" in dat)
	{
		this.descripcionCorta=dat.descripcionCorta;
	}
	this.conexiones={};
...
Escenario.prototype.mostrar_informacion_reducida=function(cadenaDeSalida)
{
	...
	texto=funcionesDeSalida.obtener_texto(this.descripcionCorta);
	cadenaDeSalida.push({tipo:"descripcion_escenario",cadena:texto});
	return cadenaDeSalida;
}
...
Escenario.prototype.mostrar_salidas_vertical=function(cadenaDeSalida)
{
	...
	for (var a=0;a<keys.length;a++)
	{
		if(this.conexiones[keys[a]].visible)
		{
			...
			cadenaDeSalida.push({tipo:"salidas_opcion",cadena:sali});				
		}
	}
	return cadenaDeSalida;
}
...
Escenario.prototype.mostrar_salidas_horizontal=function(cadenaDeSalida)
{
	...
	for (var a=0;a<keys.length;a++)
	{
		if(this.conexiones[keys[a]].visible)
		{
			if(a==keys.length-2)
			{
				if(this.conexiones[keys[a+1]].visible)
				{
					final=" "+guion.frasesInternas["9"];
				}
				else
